$(function () {
  ...
  var $parent = $(".change_btn span").parents("div.v_show");//根据当前点击元素获取到父元素
  var $v_content_list = $parent.find("div.v_content_list"); //寻找到“视频内容展示区域”
  ...
  $("span.next").click(function () {    //绑定click事件
    ...
    if (!$v_content_list.is(":animated")) {    //判断“视频内容展示区域”是否正在处于动画
      if (page == page_count) {  //已经到最后一个版面了,如果再向后，必须跳转到第一个版面
        ...
        $v_content_list.animate({ left: '-=' + v_width }, "slow");  //通过改变left值，达到每次换一个版面
        page++;
      }
  ...
  $("span.prev").click(function () {
    ...
    if (!$v_content_list.is(":animated")) {    //判断“视频内容展示区域”是否正在处于动画
      if (page == 1) {  //已经到第一个版面了,如果再向前，必须跳转到最后一个版面。
        ...
        $v_content_list.animate({ left: '+=' + v_width }, "slow");
        page--;
      }
      $parent.find("span").eq((page - 1)).addClass("current").siblings().removeClass("current");
    }
